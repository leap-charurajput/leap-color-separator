...
function getServerBasePath() {
	...
	try {
		...
		const settingsPath = path.join(homeDir, 'Documents', 'LEAP Settings', 'logobaseDataPathSettings.json')
...
async function getColorCodesFromExcel(teamCode, documentPath) {
	try {
		...
		try {
			...
			throw new Error(`Failed to read Excel file: Cannot access file ${excelFilePath}. The file may be open in another application, corrupted, or locked. ${readError.message}`)
		}
...
async function getProfileInformation(profileCode) {
	try {
		...
		const flashValue = matchedProfile['Flash'] ? String(matchedProfile['Flash']).trim().toUpperCase() : ''
		const coolValue = matchedProfile['Cool'] ? String(matchedProfile['Cool']).trim().toUpperCase() : ''
...
async function getInkInformation(inkName, profileName) {
	try {
			...
			if (rowData && rowData[inkColorColIndex]) {
				...
				if (inkColorMatches) {
					if (profileNameUpper && profileColIndex !== -1) {
						const excelProfileName = rowData[profileColIndex] ? String(rowData[profileColIndex]).trim().toUpperCase() : ''
		...
		const meshValue = matchedRow[colorMeshColIndex] ? String(matchedRow[colorMeshColIndex]).trim() : '110'
		const twoHitsValue = matchedRow[twoHitsColIndex] ? String(matchedRow[twoHitsColIndex]).trim().toUpperCase() : 'N'
		const twoHits = twoHitsValue === 'Y' || twoHitsValue === 'YES'
		const matchedProfileName = profileColIndex !== -1 && matchedRow[profileColIndex] ? String(matchedRow[profileColIndex]).trim() : null
		...
		if (profileName) {
			if (typeof profileName === 'string') {
				...
				if (profileName.length !== inkNames.length) {
					throw new Error(`Profile names array length (${profileName.length}) must match ink names array length (${inkNames.length})`)
				}
