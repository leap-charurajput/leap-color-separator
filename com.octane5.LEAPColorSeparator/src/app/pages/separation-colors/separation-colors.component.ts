...
export class SeparationColorsComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
	...
	checkIfSeparatedDocument(): void {
		...
		this.controller.checkSeparatedDocument()
			.then(result => {
				...
				if (data.isSeparatedDoc) {
					...
					if (data.leapSeparationColorsData && Array.isArray(data.leapSeparationColorsData) && data.leapSeparationColorsData.length > 0) {
						console.log('[SEPARATION] Found LEAPSeparationColorsData in XMP:', data.leapSeparationColorsData.length, 'rows');
						this.isLoadingSwatches = true;
						...
						if (colorRowsFromXMP && colorRowsFromXMP.length > 0) {
							...
							console.log('[SEPARATION] Loaded color rows from XMP data on document check:', colorRowsFromXMP.length, 'rows');
							...
							console.log('[SEPARATION] Should show table?', !this.isLoadingSwatches && (this.isSeparatedDoc || (!this.isLoadingGraphics && this.graphicOptions.length > 0)));
						} else {
						...
						console.log('[SEPARATION] No LEAPSeparationColorsData found in XMP, will use SeparatedLayerNames + Excel');
						const separatedLayerNames = data.separatedLayerNames;
						if (separatedLayerNames && Array.isArray(separatedLayerNames) && separatedLayerNames.length > 0) {
							this.isLoadingSwatches = true;
	...
	convertXMPDataToColorRows(xmpData: any[]): ColorRow[] | null {
		...
		console.log('[SEPARATION] Converting XMP data directly (with hex stored):', xmpData.length, 'rows');
	...
	loadColorRowsFromSeparatedLayerNames(): void {
		...
		console.log('[SEPARATION] Loading color rows using SeparatedLayerNames + Excel for graphic:', this.selectedGraphic);
		this.isLoadingSwatches = true;

		this.controller.getGraphicSwatches(this.selectedGraphic)
			.then(result => {
				if (result.success && result.swatches && Array.isArray(result.swatches)) {
					console.log('[SEPARATION] Fetched swatches from SeparatedLayerNames:', result.swatches.map((s: any) => s.name));

					const validSwatches = this.filterValidSwatches(result.swatches);
					console.log('[SEPARATION] Valid swatches (exist in document):', validSwatches.map((s: any) => s.name));
					...
					console.error('[SEPARATION] Failed to load swatches from SeparatedLayerNames:', result.error || 'Invalid response');
					this.graphicSwatches = [];
					this.colorRows = [];
					this.isLoadingSwatches = false;
			...
			.then(inkResult => {
				...
				if (needsFallbackProfile && this.documentProfileMetadata) {
					...
					const profileCode = this.documentProfileMetadata.profileCode || this.documentProfileMetadata.profileName;
					if (profileCode) {
				...
				return fallbackProfilePromise.then((fallbackProfileResult: any) => {
					const fallbackProfile = fallbackProfileResult && fallbackProfileResult.success && fallbackProfileResult.profileInfo && fallbackProfileResult.profileInfo.found
					...
					this.graphicSwatches.forEach((swatchData: any, index: number) => {
						...
						const firstRow = this.createColorRowFromSwatch(swatchData, inkInfo, fallbackProfile, currentId++);
					...
					console.log('[SEPARATION] Color rows loaded from SeparatedLayerNames + Excel:', sortedColorRows.length, 'rows');
					console.log('[SEPARATION] Color rows after sorting:', sortedColorRows.map(r => r.colorName));
	...
	private filterValidSwatches(swatches: any[]): any[] {
		return swatches.filter(swatch => {
			...
			if (!hasValidColor) {
				console.log(`[SEPARATION] Filtering out swatch "${swatch.name}" - not found in document (no CMYK/RGB data)`);
			}
			return hasValidColor;
		});
	}

	private createColorRowFromSwatch(swatchData: any, inkInfo: any, fallbackProfile: any, currentId: number): ColorRow {
		const colorHex = swatchData.hex || this.getRandomColor();
		const isWhiteUBColor = this.isWhiteUB(swatchData.name);
	...
	private createColorRowsFromSwatchesWithDefaults(): void {
		...
		const sortedColorRows = this.sortColorRowsWithWhiteUBAtBottom(newColorRows);
		console.log('[SEPARATION] Color rows after sorting (fallback):', sortedColorRows.map(r => r.colorName));
		this.colorRows = sortedColorRows;
	...
	handleExportSeparations(exportOptions: any): void {
		...
		if (exportOptions.exportPrintGuide) {
			this.controller.exportPrintGuidePDF()
				.then(result => {
		...
		if (exportOptions.exportPostscript) {
			setTimeout(() => {
				this.controller.exportPostscript()
					.then(result => {
						if (result && result.success) {
		...
		if (exportOptions.exportSeparationsPreview) {
			setTimeout(() => {
				if (exportOptions.exportPostscript) {
					setTimeout(() => {
						this.controller.exportSeparationsPreviewPDF()
							.then(result => {
								if (result && result.success) {
							...
							.catch(err => {
								exportErrors.push('Separations Preview PDF: ' + (err.message || err.reason || 'Unknown error'));
								this.checkExportCompletion(exportOptions, exportResults, exportErrors);
							});
					}, 500);
				} else {
	...
	private checkExportCompletion(exportOptions: any, exportResults: string[], exportErrors: string[]): void {
		const totalExports = (exportOptions.exportPrintGuide ? 1 : 0) +
		...
		if (exportResults.length + exportErrors.length >= totalExports) {
			if (exportErrors.length > 0 && exportResults.length === 0) {
				...
				const successMessage = exportResults.length === 1
	...
	getUniqueSecondHitName(currentName: string): string {
		...
		const existingNames = new Set(this.colorRows.map(row => (row.colorName || '').trim().toLowerCase()));
	...
	updateSepTableInDocument(): void {
		...
		this.controller.updateSepTable(separationData)
			.then(result => {
				if (result.success) {
