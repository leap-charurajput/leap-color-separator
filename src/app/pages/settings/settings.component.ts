...
export class SettingsComponent implements OnInit, OnChanges {
	...
	constructor(private controller: ControllerService, private dataIssues: DataIssuesService) { }
	...
	onEnvironmentChange(envKey: keyof typeof this.environments): void {
		...
		this.controller.saveAppVersion(this.selectedEnvironmentUrl).then((result) => {
			if (result) {
				...
				/* Error goes to the banner + log, not a modal (alerts are reserved for direct user guidance). */
				console.error('[Settings] Failed to save environment URL');
				this.dataIssues.report('settings-env-url', 'Failed to save the environment URL.');
			}
		});
	}
