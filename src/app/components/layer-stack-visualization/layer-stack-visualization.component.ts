...
export class LayerStackVisualizationComponent implements OnInit, OnDestroy, OnChanges {
 ...
 get garmentLabelText(): string {
  const styleDesc = this.styleInfo?.['Style Desc'] || this.styleInfo?.['Style  Desc'] || this.styleInfo?.['StyleDesc'] || '';
  const garmentType = styleDesc || this.activeGarment.label || 'Base';
  const colorName = this.bodyColorName || this.bodyColorNameFromDocument || '';

  if (garmentType && colorName && garmentType.toLowerCase() !== colorName.toLowerCase()) {
   return `${garmentType} (${colorName})`;
  }
  return garmentType || colorName || '';
 }
